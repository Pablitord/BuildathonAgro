...
export async function completarInformacion(operacionId: string, datos: string, equivalenciaKgPorUnidad?: number): Promise<Operacion> {
  const previa = getOperacion(operacionId);
  if (!previa) throw new Error("Operación no encontrada");
  const textoCombinado = `${previa.texto_original}\nInformación adicional: ${datos}`;
  const nuevo = await crearPlan(textoCombinado, equivalenciaKgPorUnidad);
  const merged = { ...nuevo.lote, ubicacion: nuevo.lote.ubicacion ?? previa.lote.ubicacion, tiene_transporte: nuevo.lote.tiene_transporte ?? previa.lote.tiene_transporte, precio_minimo_kg: nuevo.lote.precio_minimo_kg ?? previa.lote.precio_minimo_kg, plazo_declarado: nuevo.lote.plazo_declarado ?? previa.lote.plazo_declarado, horas_limite: nuevo.lote.horas_limite ?? previa.lote.horas_limite, almacenamiento: nuevo.lote.almacenamiento ?? previa.lote.almacenamiento, calidad: nuevo.lote.calidad === "desconocida" ? previa.lote.calidad : nuevo.lote.calidad };
  const urgencia = merged.cantidad_kg ? calcularUrgencia(merged) : null;
  const buyers = merged.cantidad_kg ? buscarCompradores(merged, previa.compradores_excluidos) : [];
  const distribucion = buyers.length ? optimizarDistribucion(merged, buyers) : null;
  const estado: Operacion["estado"] = !merged.cantidad_kg ? "requiere_informacion" : !buyers.length ? "sin_coincidencias" : "pendiente_aprobacion";
  const updated = updateOperacion(operacionId, () => ({ ...nuevo, id: operacionId, lote: merged, urgencia, distribucion, compradores_encontrados: buyers.length, compradores_excluidos: previa.compradores_excluidos, estado, texto_original: previa.texto_original, historial: [...previa.historial, registrarDecision({ tipo: estado === "requiere_informacion" ? "requiere_informacion" : estado === "sin_coincidencias" ? "sin_coincidencias" : "plan_inicial", detalle: "Información complementaria incorporada sin reiniciar el lote.", snapshot: { lote: merged, distribucion } })], traza: [...previa.traza, ...nuevo.traza, registrarTraza({ herramienta: "completar_informacion", entrada: { datos }, salida: { estado } })] }));
  if (!updated) throw new Error("No se pudo completar la operación"); return updated;
}
